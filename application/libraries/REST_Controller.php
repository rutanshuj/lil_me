...
abstract class REST_Controller extends MX_Controller
{
    ...
    */
	...
	/***********************************************************************
	| New method to get / set access token
	|
	/**********************************************************************/
		public function set_access_token($user_id=false){
			if($user_id){
				$uuid=false;
				...
				$access_token=str_replace("}",'',(str_replace("{",'',$uuid)));
				$db_data['access_token']=$access_token;
				$this->db->where('id',$user_id)->update('users_cms',$db_data);
				return 	$access_token;	
			}
			return false;
		}
		
		public function check_access_token($user_id,$access_token){
			$access_token=$this->db->select('access_token')->where(array('id'=>$user_id,'access_token'=>$access_token))->get('users_cms')->result_array();
			if($access_token){
				return true;
			}
			return false;
		}
		public function delete_access_token($user_id=false){
			if($user_id){
		...
		public function get_access_token($user_id=false){ 
			if($user_id){
				$tmp_access_token=$this->db->select('access_token')->where('id',$user_id)->get('users_cms')->result_array();
				if($tmp_access_token){
					return 	$tmp_access_token[0]['access_token'];	
				}
			}
			return false;				
		}
