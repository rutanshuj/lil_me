...
class REST
{
    ...
    public function initialize($config)
    {
        ...
        isset($config['ACCESS_TOKEN']) && $this->access_token = $config['ACCESS_TOKEN'];
        isset($config['USER_ID']) && $this->user_id = $config['USER_ID'];
    ...
    protected function _call($method, $uri, $params = array(), $format = NULL)
    {
        ...
        {
            $this->_ci->curl->http_header('ACCESS-TOKEN', $this->access_token);
        }		
		
		//set the USER_ID
        if ($this->user_id != '')
        {
            $this->_ci->curl->http_header('USER-ID', $this->user_id);
        }		
     ...
     *
     * @access  public
     * @author  Vaibhav Malushte
     * @version 1.0
     */
	public function beautify($data){
