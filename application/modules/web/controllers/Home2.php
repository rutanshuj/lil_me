...
class Home2 extends CI_Controller {
	...
	function index()
	{
		...
		$api_key ='fb0aa13efb9ac71e1c09094d7102d798';// $this->session->userdata('api_key');
	...
	function products_by_param()
	{
		...
		$api_key = $this->session->userdata('api_key');
		$data['category_id']=$category_id=trim($this->input->get('category_id',true));
		//$data['category_idq']=$category_id=trim($this->input->post('category_id',true));
		...
		$data['cart_list']=$this->Cart_model->get_cartList($user_id,$api_key);
		$data['category_list'] = $this->Category_model->product_category();
		$data['page_no']=$page_no=$this->input->post('page_no',true);
		$config["per_page"] = 8;   
        $page = ($page_no)*$config["per_page"];
		if(empty($page_no)||is_null($page_no)|| !isset($page_no))
        {
			$page=1;
        }
		...
		{
			$data['product_list'] = $this->Pagination_Model->get_products_criteriaWise
			($category_id,$size_id,$gender,$sortflag,$sortparam,$config["per_page"], $page);
			if($sortflag==0)
			{
				$data['sortflag']=1;
			}
