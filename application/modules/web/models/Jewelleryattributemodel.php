...
class Jewelleryattributemodel extends CI_Model
{
	...
	function getGroupOptions()
   {
     ...
     $result=$query_getOptions->result();
     return $result;
   }
   function get_size($product_id=false)
   {
	 $this->db->select('*')
     ->from('master_size m');
	if(is_numeric($product_id) && isset($product_id)){
	$this->db->where('pom.product_id',$product_id);	
	$this->db->join('product_option_mapper pom','pom.size_id=m.size_id');
	}
   ...
   function getProductOptionList($product_id,$attribute_id)
   {
   	 $whereArr=array('av.attribute_id'=>$attribute_id,'av.product_id'=>$product_id);
   	 $this->db->select('attribute_name,attribute_value')
       ->from('product p')
