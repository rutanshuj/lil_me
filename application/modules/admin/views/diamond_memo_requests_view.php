      ...
      <div class="content-wrapper">
        ...
        <section class="content">
		  ...
		  <div class="col-xs-12">
					...
					<div style="float: left;padding-right: 10px;padding-bottom: 10px;"><a href="<?php echo base_url('admin/diamond_out_on_memo/memo_requests');?>" class="btn btn-primary"" class="btn btn-primary" style="border-style:groove solid double;border-color: black;border-width: 2px;border-radius: 6px"> Memo Requests</a></div>
 ...
 <link rel="stylesheet" type="text/css" href="<?php echo base_url();?>assets/plugins/sweetalert-master/dist/sweetalert.css">
				...
				function approve(memo_requests_date,expiry_date,memo_id,product_name,email,product_id,fullname,status){
					...
					if(expiry_date==''){
						...
						$.ajax({
							type: "POST",
							url: "<?php echo base_url('admin/diamond_out_on_memo/approve');?>",
							data: {                
							...
							success: function(result){
							  ...
							  // alert('Memo request has been approve till '+expiry_date+' for project '+product_name);
							   window.location = "<?php echo base_url('admin/diamond_out_on_memo/memo_requests');?>";
							}
						});
                ...
                <table id="example2" class="table table-bordered table-striped">
					...
					<?php if(count($memo_requests)!="0"){
						...
						foreach($memo_requests as $memo_requests_rows){
							...
							$expiry_date="";
							if(isset($memo_requests_rows->expiry_date)&&($memo_requests_rows->expiry_date!="")&&($memo_requests_rows->expiry_date!="0000-00-00 00:00:00")){
						...
						<input type="textbox" name="memo_requests_date_<?php echo $memo_limit;?>" id="memo_requests_date_<?php echo $memo_limit;?>" value="<?php echo $memo_request_date;?>">
						
						</td>
						<td><input type="textbox" name="expiry_date_<?php echo $memo_limit;?>" id="expiry_date_<?php echo $memo_limit;?>" value="<?php echo $expiry_date;?>"></td>
						...
						<td><a href="<?php echo base_url('admin/diamond_out_on_memo/reject?id=').$memo_requests_rows->out_on_memo_id."&name=".$memo_requests_rows->username."&email=".$memo_requests_rows->email_id."&fullname=".$full_name;?>" class="btn btn-primary" style="max-width: 80px;text-align: center;">REJECT</a></td>
