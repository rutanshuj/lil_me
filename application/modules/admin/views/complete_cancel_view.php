        ...
        <section class="content">
		  ...
		  <div class="col-xs-12">
				...
				/* 	
					...
					<div style="float: left;padding-right:10px;padding-bottom: 10px;"><a href="<?php echo base_url('admin/sales/complete_cancel');?>"  style="border-style:groove solid double;border-color: black;border-width: 2px;border-radius: 6px" class="btn btn-primary" id="">Complete & Cancel</a></div>
            ...
            <div class="col-xs-12">
			  ...
			  <form role="form" name="product_add" action="<?php echo base_url('admin/sales/complete_cancel');?>" method="get" onsubmit="return validateForm()" >
...
<?php 
				foreach($sales_status as $sales_status_rows){			
					...
					if(($sales_status_rows['sales_status']!="cart")&&($sales_status_rows['sales_status']!="in process")&&($sales_status_rows['sales_status']!="dispatched")){
		...
		<input type="text" class="form-control" name="end_date"id="end_date"style="height: 31px;"  value="<?php echo $end_date;?>" placeholder="End date">
				...
				<a href="<?php echo base_url('admin/sales/complete_cancel');?>" class="btn btn-primary">Clear</a></div>
               </div>
...
function update_status(user_id,status,card_id,user_select_id,limit_plus_minus,u_p_id,u_s_id,start_date,end_date,email,product_name){	
					  ...
					  function(isConfirm){  
						  if (isConfirm) { 
							...
							ajaxRequest.done(function (response, textStatus, jqXHR){
								 ...
								 if(response=="1"){								 
									window.location = "<?php echo base_url('admin/sales/complete_cancel?id=');?>"+user_select_id+"&div_plus_minus="+limit_plus_minus+"&u_p_id="+u_p_id+"&u_s_id="+u_s_id+"&start_date="+start_date+"&end_date="+end_date;
								 } else {									
		   ...
		   <?php 
		   $limit_plus_minus="0";
		   foreach($user_ids as $user_rows){ $num="0";
			...
			<table id="example7" class="table table-bordered table-striped" >
					...
					?>
                      ...
                      <tr>
						...
						if(($sales_details_rows['order_tracker_order_status']=="delivered")||($sales_details_rows['order_tracker_order_status']=="cancelled")){
					?>
