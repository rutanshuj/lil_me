      ...
      <div class="content-wrapper">
        ...
        <section class="content">
		  ...
		  <div class="col-xs-12">
					...
					<div style="float: left;padding-right: 10px;padding-bottom: 10px;"><a href="<?php echo base_url('admin/product_download_excel');?>" class="btn btn-primary"" class="btn btn-primary" id="jewellery_download_excel_page"> Download Excel</a></div>
					<div style="float: left;padding-bottom: 10px;"><a href="<?php echo base_url('admin/product_bulk_image_upload');?>" class="btn btn-primary" id="jewellery_bulk_image_upload_page"> Bulk Image Upload</a></div>
				</div>
            <div class="col-xs-12">
              <div class="box">
...
//document.getElementById('diamond_upload').style.pointerEvents = 'none';
//document.getElementById('diamond_back').style.pointerEvents = 'none';
...
function submitForm() {
			...
			showLoading();
            $.ajax({
              url: "<?php echo base_url('admin/excel_uploader/excel_upload');?>",
              type: "POST",
              data: fd,
              processData: false, 
              contentType: false   
				...
				window.location = "<?php echo base_url('admin/excel_uploader');?>";
            });
            return false;
        }
function submitForm1(name){
		   ...
		   $.ajax({
            ...
            <div style="float:left" ><input value="Upload"  type="button" id="diamond_upload" class="btn btn-primary" onClick="return submitForm1(document.getElementById('file_full_path').value)" /></div>
			
			<input type="hidden" id="file_full_path" name="file_full_path" value="<?php if(isset($file_full_path)){echo $file_full_path;}?>">
			
			<a href="<?php echo base_url('admin/excel_uploader');?>"  id="diamond_back" class="btn btn-primary" >Back</a>			
			...
			?>
			<div class="col-xs-6">
			<div style="float: left;"><span><input id="radio_upload" type="radio" name="u_load" checked></span><span style="padding-left: 10px;">Direct Upload</span></div>
			
			<div ><span style="padding-left: 30px;"><input id="radio_upload_direct" type="radio" name="u_load" ></span><span style="padding-left: 10px;">Verify and Upload<span></div>
     ...
     <b>Select File To Upload:</b><br />
			<div style="float:left;padding-top: 20px;"><input type="file" name="userfile" accept= "application/vnd.ms-excel,application/vnd.openxmlformats-officedocument.spreadsheetml.sheet" required/></div>
			<div style="padding-top: 20px;"><input type="submit" name="submit" value="Upload" class="btn btn-primary" /></div>
		...
		<form  id="frm_verify_direct" action="<?php echo base_url().('admin/excel_uploader/excel_verify_upload');?>" method="POST" enctype="multipart/form-data" >          
            	<b>Select File To Upload:</b><br />
			<div style="float:left;padding-top: 20px;"><input type="file" name="userfile" accept= "application/vnd.ms-excel,application/vnd.openxmlformats-officedocument.spreadsheetml.sheet"/></div>
			<div style="padding-top: 20px;"><input type="submit" name="submit" value="Upload" class="btn btn-primary" id="diamond_upload"/></div>
			<input type="hidden" id="diamond_back">
...
<div style="overflow-y:hidden" class="col-xs-12"	 ><?php echo $table;?></div>
<?php }?>
