...
/// check email  stock 
class Bulk_certificate_image_upload extends Admin_Controller {
	...
	public function __construct() {		
		...
		$this->load->view('certificate_bulkupload_view', $data);

		$this->load->view('footer', $data);
      
    }
function zipupload_diamond_certificate()
	{


	$zip=$_FILES["userfile"]["tmp_name"];
	$this->unzip->allow(array('css', 'js', 'png', 'gif', 'jpeg', 'jpg', 'tpl', 'html', 'swf','JPG','PNG','Gif'));
	$data=$this->unzip->extract($zip,"./assets/img/diamond_certificate");
	$filenames=$data['filename'];
	$file_location=$data['file_locations'];
	$count=0;
		for($i = 0; $i < count($filenames); $i++) {
			$imageArr=explode(".",$filenames[$i]);
			$product_id=$this->Diamondmodel->getcertName($imageArr[0]);
			//echo $product_id;
            ...
            ->from('certificate_images_diamond')
    		...
    		{ 
			   ...
			   'updated_by'=>$this->username);
				if($this->db->insert('certificate_images_diamond', $insertData))
				{
					$count++;
				} 
			...
			foreach ($query_getimage->result() as $key=>$img) {
                ...
                $this->db->update('certificate_images_diamond',$updateData); 
	 ...
	 //redirect('admin/bulk_certificate_image_upload');
