	...
	public function add(){
		...
		if ($this->form_validation->run() == True){
			...
			if (!empty($_FILES['userfile']['name'])) {
				...
				$config['file_name'] = md5($data_save['category_name'].date("Y-M-d-i"));
			...
			if ($this->form_validation->run() == True){
				if (!empty($_FILES['userfile']['name'])) {
					...
					foreach($category_detail as $category_row){						
						$del_image_image_url = $category_row->image_url;			
						@unlink($del_image_image_url);
						$del_image_thumbnail_url = $category_row->image_thumbnail_url;
						@unlink($del_image_thumbnail_url);						
					}
					//////////////
					...
					$config['max_width']  = CATEGORY_IMAGE_MAX_WIDTH;				
					$config['max_height']  = CATEGORY_IMAGE_MAX_HEIGHT;
					$config['file_name'] = md5($data['category_name'].date("Y-M-d-i"));
					$this->load->library('upload', $config);
					if ( ! $this->upload->do_upload()) {				
