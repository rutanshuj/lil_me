      ...
      <div class="content-wrapper">
        ...
        <section class="content">
		  ...
		  <div class="col-xs-12">
					...
					<div style="float: left;padding-right: 10px;padding-bottom: 10px;"><a href="<?php echo base_url('admin/diamond_out_on_memo/on_memo_status');?>" class="btn btn-primary" style="border-style:groove solid double;border-color: black;border-width: 2px;border-radius: 6px"> On Memo Status</a></div>
		...
		?>
				...
				<script>
				function dia_extend(date_id,id,product_name){
					if(date_id==''){
						...
						$.ajax({
							type: "POST",
							url: "<?php echo base_url('admin/diamond_out_on_memo/diamond_extend');?>",
							data: {                
								'date_id' : date_id,
								'id' : id,
							},
					...
					<?php if(count($memo_history)!="0"){
						...
						<?php
							...
							if(isset($memo_history_rows->request_approve_date)&&($memo_history_rows->request_approve_date!="")){
								//echo $memo_history_rows->request_approve_date;
						...
						?>
						<input type="textbox" name="datepicker_<?php echo $limit;?>" id="datepicker_<?php echo $limit;?>" value="<?php echo $expiry_date;?>" class="form-control" style=" max-width: 100px;">
						...
						<td><a href="<?php echo base_url('admin/diamond_out_on_memo/on_memo_status_sole?id=').$memo_history_rows->out_on_memo_id."&name=".$memo_history_rows->username."&product_id=".$memo_history_rows->product_id;?>" class="btn btn-primary" style="min-width: 54px;">Sold</a></td>
						<td><a href="<?php echo base_url('admin/diamond_out_on_memo/on_memo_status_return?id=').$memo_history_rows->out_on_memo_id."&name=".$memo_history_rows->username;?>" class="btn btn-primary" style="min-width: 54px;">Return</a></td>
						<td>
						
						<input value="Extend" type="button"  class="btn btn-primary" onClick="return dia_extend(document.getElementById('datepicker_<?php echo $limit;?>').value,'<?php echo $memo_history_rows->out_on_memo_id;?>','<?php echo $memo_history_rows->product_name;?>')" style="min-width: 54px;"/>​
