...
/// check email  stock
class Diamond_out_on_memo extends Admin_Controller {
	public $page='diamond_out_on_memo';
	public function __construct() {		
		...
		$this->load->Model('Diamond_out_on_memo_model');
	...
	public function reject(){
		...
		if(is_numeric($out_on_memo_id)){			
			...
			$this->Diamond_out_on_memo_model->memo_reject($out_on_memo_id,$data_update);
			...
			redirect('admin/diamond_out_on_memo/memo_requests');	
		}
	...
	public function index(){
		...
		$data['due_today'] = $this->Diamond_out_on_memo_model->get_diamond_due_memo($current_date,'request');
		
		
		$data['out_on_memo'] = $this->Diamond_out_on_memo_model->get_diamond_due_memo(Null,'approve','1');
		/* echo"<pre>";
		...
		$this->load->view('diamond_memo_dashboard_view', $data);
	...
	public function memo_requests(){
		...
		$id = $this->session->userdata('id');
		$data['memo_requests'] = $this->Diamond_out_on_memo_model->get_diamond_Due_memo(null,'request',null);
		$data['total_memo_request']=count($data['memo_requests']);
		
		
		/* echo"<pre>";
		print_r($data['memo_requests']);
		exit; */
		...
		$this->load->view('sub_header', $data);		
		$this->load->view('sidebar', $data);
		$this->load->view('diamond_memo_requests_view', $data);
	...
	public function on_memo_status_return(){
		...
		if(is_numeric($out_on_memo_id)){			
			...
			redirect('admin/diamond_out_on_memo/on_memo_status');	
		}
	}
	...
	public function on_memo_status_sole(){
		...
		if(is_numeric($out_on_memo_id)){			
			...
			$this->db->update('product_diamond', $product_update); 
	...
	public function approve(){
		...
		if(isset($memo_requests_date) && isset($memo_id)){
			...
			$this->db->update('out_on_memo_diamond',$data_update); 
	...
	public function diamond_extend(){
	...
	public function on_memo_status(){
		...
		/// approve , request =1
		$data['memo_history'] = $this->Diamond_out_on_memo_model->get_diamond_Due_memo(Null,'approve','1');
		
		$data['total_memo_history'] = count($data['memo_history']);
		...
		$this->load->view('sub_header', $data);		
		$this->load->view('sidebar', $data);
		$this->load->view('diamond_on_memo_status_view', $data);
	...
	public function memo_history(){
		...
		$data['memo_history'] = $this->Diamond_out_on_memo_model->get_diamond_Due_memo(Null,Null,'0');
		/* echo"<pre>";
		print_r($data['memo_history']);
		exit; */
		$this->load->view('header', $data);
		$this->load->view('sub_header', $data);		
		$this->load->view('sidebar', $data);
		$this->load->view('diamond_memo_history_view', $data);
		// memo_history_view
		// diamond_memo_history_view
		$this->load->view('footer', $data);
	}
	...
	public function manual_add_select_product(){
		...
		if(!is_numeric($data['out_on_memo_user_id'])){
			redirect('admin/diamond_out_on_memo/manual_add');
		}
		$data['page'] = $this->page;
		...
		if(array($product_id)&&(isset($submit))){
			if(!empty($product_id)){
				...
				redirect('admin/diamond_out_on_memo/manual_add_make_memo');
			} else {
		...
		exit;  */
		$data['step_two_product'] = $this->Diamond_out_on_memo_model->step_two_product();
		/*  echo "<pre>";
		...
		$this->load->view('sub_header', $data);		
		$this->load->view('sidebar', $data);
		$this->load->view('diamond_manual_add_step_second_view', $data);
	...
	public function manual_add_make_memo(){
		...
		if($submit){
			...
			if (($this->form_validation->run() == True)&&(empty($data['error']))){
					...
					redirect('admin/diamond_out_on_memo/manual_add_confirmation');
		...
		$this->load->view('sub_header', $data);		
		$this->load->view('sidebar', $data);
		$this->load->view('diamond_manual_add_make_memo_view', $data);
	...
	public function memo_sent(){
		...
		$this->load->view('sub_header', $data);		
		$this->load->view('sidebar', $data);
		$this->load->view('diamond_manual_add_memo_sent_view', $data);
		/// manual_add_memo_sent_view
		/// diamond_manual_add_memo_sent_view
		$this->load->view('footer', $data);
	}
	...
	public function manual_add_confirmation(){
		...
		$data['user_details'] = $this->Diamond_out_on_memo_model->user_details_get($data['out_on_memo_user_id']);
		...
		$result =$this->db->select('product_name')->where_in('product_id', $data['out_on_memo_product_id'])->get('product_diamond')->result();
		...
		if($submit){
			...
			$this->db->update_batch('product_diamond',$product_status_update, 'product_id');
			...
			if(count($data_save)>0){
				...
				$this->db->insert_batch('out_on_memo_diamond',$data_save);
				...
				redirect('admin/diamond_out_on_memo/memo_sent');	
		...
		$this->load->view('sub_header', $data);		
		$this->load->view('sidebar', $data);
		$this->load->view('diamond_manual_add_confirmation_view', $data);
		// diamond_manual_add_confirmation_view
	 ...
	 public function manual_add(){
		...
		$data['message'] = $this->session->flashdata('message');
		$id = $this->session->userdata('id');
		$data['user_details'] = $this->Diamond_out_on_memo_model->user_details_get();
		$user_id = $this->input->post('user_id',true);
		$submit = $this->input->post('submit',true);
		if(isset($submit)&&(is_numeric($user_id))){
			$this->session->set_userdata('out_on_memo_user_id', $user_id);
			redirect('admin/diamond_out_on_memo/manual_add_select_product');
		}
		$data['out_on_memo_user_id'] = $this->session->userdata('out_on_memo_user_id');
		...
		$this->load->view('sub_header', $data);		
		$this->load->view('sidebar', $data);
		$this->load->view('diamond_manual_add_step_first_view', $data);
