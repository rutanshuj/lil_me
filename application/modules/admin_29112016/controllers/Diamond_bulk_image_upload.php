...
/// check email  stock 
class Diamond_bulk_image_upload extends Admin_Controller {
	...
	public function __construct() {		
		...
		$this->load->view('diamond_bulk_image_upload_view', $data);

		$this->load->view('footer', $data);
      
    }
function zipupload_diamond(){
	
	
	
	$zip=$_FILES["userfile"]["tmp_name"];
	$this->unzip->allow(array('css', 'js', 'png', 'gif', 'jpeg', 'jpg', 'tpl', 'html', 'swf','JPG','PNG'));
	$data=$this->unzip->extract($zip,"./assets/img/diamond");
	$filenames=$data['filename'];
	$file_location=$data['file_locations'];
	$count=0;
			for($i = 0; $i < count($filenames); $i++) {
			$imageArr=explode(".",$filenames[$i]);
			$product_id=$this->Diamondmodel->getproductName($imageArr[0]);
			//echo $product_id;
            ...
            ->from('product_images_diamond')
            ->where('image_url',base_url().$img_url);
    		...
    		{ 
			   ...
			   'updated_by'=>$this->username);
				if($this->db->insert('product_images_diamond', $insertData))
				{
					$count++;
				} 
			...
			foreach ($query_getimage->result() as $key=>$img) {
                ...
                $this->db->update('product_images_diamond',$updateData); 
	...
	// redirect('admin/diamond_bulk_image_upload');
	...
	$data=$this->unzip->extract($zip,"./assets/img/jewel");
	...
	$count=0;
			...
			if($product_id!=0) {
				...
				$thumb_file=chop($img_url,$filenames[$i]).$imageArr[0]."_thumb.".$imageArr[1];
				copy ( $file_location[$i] , $thumb_file);
				echo $this->make_thumb($file_location[$i],$thumb_file,100);
	 ...
	 if($count=="0"){
		$this->session->set_flashdata('error', "zero image uploaded");
		 ...
		 $this->session->set_flashdata('successful', $count." images uploaded");
	 }
	 redirect('admin/diamond_bulk_image_upload');
